...
function Check-Bootloader-Version {
    ...
    if ($currentBootloaderVersion -ne $desiredBootloaderVersion) {
        ...
        fastboot flash bootloader "bootloader-oriole-$desiredBootloaderVersion.img"
...
function Check-Radio-Version {
    ...
    if ($currentRadioVersion -ne $desiredRadioVersion) {
        ...
        fastboot flash radio "radio-oriole-$desiredRadioVersion.img"
        fastboot reboot-bootloader
        Start-Sleep -Seconds 5
    } else {
...
if ($wipeOption.ToLower() -eq "yes") {
    ...
    $updateResult = fastboot -w update "risingOS-2.0-OFFICIAL-BUNDLED-oriole-fastboot.zip"
    if (-not $updateResult) {
        Display-Error-Message "Error updating risingOS. Exiting."
    }
} else {
    Write-Host "Updating without wiping the device..."
    $updateResult = fastboot update "risingOS-2.0-OFFICIAL-BUNDLED-oriole-fastboot.zip"
    if (-not $updateResult) {
        Display-Error-Message "Error updating risingOS. Exiting."
    }
}
