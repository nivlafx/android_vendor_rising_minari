...
desired_bootloader_version="slider-1.3-10780582"
desired_radio_version="g5123b-125137-231014-b-10950115"
...
if check_bootloader_version; then
  ...
  fastboot flash bootloader bootloader-oriole-$desired_bootloader_version.img || { error_message="Error flashing bootloader. Exiting."; display_error_message "$error_message"; exit 1; }
...
if check_radio_version; then
  ...
  fastboot flash radio radio-oriole-$desired_radio_version.img || { error_message="Error flashing radio. Exiting."; display_error_message "$error_message"; exit 1; }
  fastboot reboot-bootloader
  sleep 5
fi
...
if [ "$wipe_option" = "yes" ]; then
  ...
  fastboot -w update risingOS-2.0-OFFICIAL-BUNDLED-oriole-fastboot.zip || { error_message="Error updating risingOS. Exiting."; display_error_message "$error_message"; exit 1; }
else
  echo "Updating without wiping the device..."
  fastboot update risingOS-2.0-OFFICIAL-BUNDLED-oriole-fastboot.zip || { error_message="Error updating risingOS. Exiting."; display_error_message "$error_message"; exit 1; }
fi
