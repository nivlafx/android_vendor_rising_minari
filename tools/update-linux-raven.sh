...
if check_bootloader_version; then
  ...
  fastboot flash bootloader bootloader-raven-$desired_bootloader_version.img || { error_message="Error flashing bootloader. Exiting."; display_error_message "$error_message"; exit 1; }
...
if check_radio_version; then
  ...
  fastboot flash radio radio-raven-$desired_radio_version.img || { error_message="Error flashing radio. Exiting."; display_error_message "$error_message"; exit 1; }
  fastboot reboot-bootloader
  sleep 5
fi
...
if [ "$wipe_option" = "yes" ]; then
  ...
  fastboot -w update risingOS-2.0-OFFICIAL-BUNDLED-raven-fastboot.zip || { error_message="Error updating risingOS. Exiting."; display_error_message "$error_message"; exit 1; }
else
  echo "Updating without wiping the device..."
  fastboot update risingOS-2.0-OFFICIAL-BUNDLED-raven-fastboot.zip || { error_message="Error updating risingOS. Exiting."; display_error_message "$error_message"; exit 1; }
fi
